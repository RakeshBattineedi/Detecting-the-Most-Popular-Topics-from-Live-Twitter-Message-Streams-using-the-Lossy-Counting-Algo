...
public class LossyCounting extends BaseRichBolt {
    ...
    public void execute(Tuple tuple) {
        ...
        Date nw = new Date(startTime);
        System.out.println("Word:" + word + ", Time: " + nw);
        collector.ack(tuple);
        lossyCounting(word);
    }
    ...
    public void lossyCounting(String word) {
        ...
        Date nww = new Date(currentTime);
        System.out.println("CurrTime: " + nww);
        if (currentTime >= startTime + 10000) {
            if (!tags.isEmpty()) {
                HashMap<String, Integer> emitL = new HashMap<String, Integer>();
                for (String str_loss : tags.keySet()) {
                    boolean b = check(str_loss);
                    if (b) {
                        Objects d = tags.get(str_loss);
                        emitL.put(str_loss, d.freq);
                    }
                }
                if (!emitL.isEmpty()) {

                    LinkedHashMap<String, Integer> sortedlist = sortHashMap(emitL);
                    Collection<String> str;
                    if (sortedlist.size() > 100) {
                        str=Collections.list(Collections.enumeration(sortedlist.keySet())).subList(0, 100);
                    }else {
                        str = sortedlist.keySet();
                    }
